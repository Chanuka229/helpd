    ...
    <div class="bg-white flex-1 relative">
        ...
        <div class="flex relative">
            <div :style="{height: 'calc(100vh - 70px)'}" class="flex-auto min-w-0">
                    ...
                    <div class="sm:flex sm:justify-between">
                        <div class="flex">
                            ...
                            <div v-on-clickaway="closeActionDropdown" class="block">
                                ...
                                <div class="relative inline-block text-left">
                                    <button class="btn p-4 rounded-none" type="button" @click="toggleActionDropdown('Companies')">
                                        <svg-vue class="h-5 w-5 text-gray-700" icon="font-awesome.users-class-regular"></svg-vue>
                                    </button>
                                    <div v-show="actions.Companies" class="origin-top-right absolute left-0 mt-1 w-56 rounded-md shadow-lg z-20">
                                        <div class="rounded-md bg-white shadow-xs">
                                            <div class="py-1">
                                                <template v-for="Companies in CompaniesList">
                                                    <a
                                                        class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 hover:text-gray-900"
                                                        href="#"
                                                        role="menuitem"
                                                        @click.prevent="action('Companies', Companies.id)"
                                                    >
                                                        {{ Companies.name }}
                                                    </a>
                                                </template>
                            ...
                            <button class="btn p-4 rounded-none" type="button" @click="deleteTicketsModal = true">
                        ...
                        <div class="flex items-center justify-end m-3 sm:m-0">
                            <div class="text-2xl font-semibold">#{{ Tickets.id }}</div>
                            <div v-if="Tickets.status" class="px-3">
                ...
                <div class="Tickets-conversation">
                    <div class="block">
                        <div class="sm:flex sm:items-center py-3 max-w-full border-b">
                            <div class="px-6 sm:pl-6 sm:pr-3 sm:flex-1 sm:w-3/4">
                                <div class="text-xl truncate">{{ Tickets.subject }}</div>
                                <template v-for="(label, index) in Tickets.labels">
                                    <div
                            ...
                            <div class="px-6 sm:pl-3 sm:pr-6 sm:flex-1 sm:w-1/4">
                                <div class="flex items-center sm:float-right mt-3 sm:mt-0">
                                    <div class="text-sm sm:pr-2">{{ Tickets.created_at | momentFormatDateTimeAgo }}</div>
                        ...
                        <div v-show="replyForm" class="px-6 py-3 border-b">
                                    ...
                                    <template v-slot:bottom>
                                        <div class="flex justify-between border border-t-0">
                                            ...
                                            <div class="flex">
                                                <select
                                                    id="status"
                                                    v-model="TicketsReply.status_id"
                                                    aria-label="Sort by"
                            ...
                            <template v-if="TicketsReply.attachments.length > 0">
                                <div class="mt-3 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2">
                                    <template v-for="(attachment, index) in TicketsReply.attachments">
                                        <attachment :details="attachment" v-on:remove="removeAttachment(index)"/>
                                    </template>
                                </div>
                            </template>
                        </div>
                        <template v-if="Tickets.TicketsReplies.length > 0">
                            <div class="block">
                                <template v-for="(TicketsReply, index) in Tickets.TicketsReplies">
                                    <div :class="{'border-t' : index > 0}" class="flex p-6">
                                        <img
                                            :alt="$t('Avatar')"
                                            :src="TicketsReply.user.avatar !== 'gravatar' ? TicketsReply.user.avatar : TicketsReply.user.gravatar"
                                        ...
                                        <div class="sm:pl-6 pb-2 w-full">
                                            <div class="md:flex md:items-center pb-1">
                                                <div class="md:flex-1 text-lg font-semibold text-gray-800">
                                                    {{ TicketsReply.user.name }}
                                                </div>
                                                <div class="md:flex-1">
                                                    <div class="md:float-right text-sm">
                                                        {{ TicketsReply.created_at | momentFormatDateTime }}
                                                    </div>
                                                </div>
                                            </div>
                                            <p class="text-gray-700 Tickets-reply-body" v-html="TicketsReply.body"/>
                                            <template v-if="TicketsReply.attachments.length > 0">
                                                <div class="mt-5 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2">
                                                    <template v-for="attachment in TicketsReply.attachments">
                            ...
                            <div class="h-full flex border-t">
                                <div class="m-auto">
                                    <div class="grid grid-cols-1 justify-items-center h-full w-full py-24">
                                        ...
                                        <div class="flex justify-center items-center">
                                            <div class="w-full font-semibold text-2xl">{{ $t('This Tickets has no conversations') }}</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
            ...
            <div :style="{flex: '0 0 240px', width: '240px'}" class="bg-gray-100 hidden lg:block border-l">
                <div v-if="Tickets.user" class="p-6">
                    <div class="flex">
                        <img
                            :alt="$t('Avatar')"
                            :src="Tickets.user.avatar !== 'gravatar' ? Tickets.user.avatar : Tickets.user.gravatar"
                            class="h-16 w-16"
                        />
                    </div>
                    <div class="mt-2">
                        <div class="text-gray-800 font-medium truncate">{{ Tickets.user.name }}</div>
                        <div class="flex items-center text-sm leading-5 text-gray-600">
                            ...
                            <span class="truncate">{{ Tickets.user.email }}</span>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div v-show="deleteTicketsModal" class="fixed z-20 inset-0 overflow-y-auto">
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                >
                    <div v-show="deleteTicketsModal" class="fixed inset-0 transition-opacity">
                    ...
                    >
                                ...
                                <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                    ...
                                    <div class="mt-2">
                                        <p class="text-sm leading-5 text-gray-500">
                                            {{ $t('Are you sure you want to delete the Tickets?') }}
                        ...
                        <div class="bg-gray-100 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                            ...
                            <button
                                ...
                                type="button"
                                @click="deleteTicketsModal = false"
                            >
                                {{ $t('Cancel') }}
                            </button>
                        </div>
...
export default {
    ...
    metaInfo() {
        return {
            title: this.$i18n.t('Manage Tickets')
        }
    },
    mounted() {
        this.getTickets();
    ...
    methods: {
        getTickets() {
            ...
            axios.get('api/dashboard/Tickets/' + self.$route.params.uuid).then(function (response) {
                ...
                self.TicketsReply.status_id = response.data.status_id;
            }).catch(function () {
        ...
        discardReply() {
            this.TicketsReply.body = '';
            this.TicketsReply.attachments = [];
        ...
        uploadFile(e) {
            ...
            axios.post(
                ...
                self.TicketsReply.attachments.push(response.data);
            }).catch(function () {
                self.loading.file = false;
                self.uploadingFileProgress = 0;
                self.$refs.fileInput.value = null;
        ...
        removeAttachment(attachment) {
            this.TicketsReply.attachments.splice(attachment, 1);
        },
        removeLabel(index) {
            const self = this;
            const label = self.Tickets.labels[index];
            axios.post('api/dashboard/Tickets/' + self.$route.params.uuid + '/remove-label', {
                label: label.id,
            }).then(function () {
                self.Tickets.labels.splice(index, 1);
            });
        },
        addReply() {
            const self = this;
            self.loading.reply = true;
            axios.post('api/dashboard/Tickets/' + self.$route.params.uuid + '/reply', self.TicketsReply).then(function (response) {
                self.$notify({
                ...
                self.TicketsReply.status_id = response.data.Tickets.status_id;
                self.discardReply();
        ...
        toggleActionDropdown(action) {
            ...
            if (action === 'Companies') {
                this.actions.Companies = !this.actions.Companies;
                this.actions.agent = false;
                this.actions.label = false;
                this.actions.priority = false;
            }
