...
use App\Http\Controllers\Api\Dashboard\TicketsController;
...
class FileController extends Controller
{
    ...
    public function download(string $uuid, Request $request): StreamedResponse
    {
        ...
        if ($file->path !== 'public') {
            ...
            $TicketsReply = $file->TicketsReply();
            if ($TicketsReply) {
                $Tickets = $TicketsReply->Tickets;
                if (!(
                    $Tickets->user_id === $file->user_id ||
                    $Tickets->agent_id === $file->user_id ||
                    $Tickets->closed_by === $file->user_id ||
                    $Tickets->user_id === $user->id ||
                    $Tickets->agent_id === $user->id ||
                    $Tickets->closed_by === $user->id ||
                    $user->userRole->checkPermission(str_replace('\\', '.', TicketsController::class))
                )) {
                    abort(404);
                }
            }
